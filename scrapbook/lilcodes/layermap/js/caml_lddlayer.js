      ...
      self.loadMarks = function(){
          ...
          for(var i=0; i<self.data.planApps.planApp.length; i++){
              ...
              if(checkIfIncluded(app, crit)){
                  var appMark;
                  if (app.geoPoly != null){
                      appMark = self.renderAppMarkAsPoly(app, crit);
                  }else{
                      appMark = self.renderAppMarkAsCircle(app,crit);
                  }
                  appMark.addTo(self.map);
                  self.mark.push(appMark);
              }
          }
      }

      self.renderAppMarkAsCircle = function(app, crit){
...
//            self.ctls[i].value  
    ...
    self.setMapMarkAppearance = function (pt, crit){
        var lineColor = 'rgb(50,90,220)';
        var fillColor = 'rgb(50,90,200)';
        var opac = 0.3;
        var rad = 12; 
        if(checkIfIncluded(pt, crit)){
           if (crit.legend = "absolute_sh"){
               if (pt.socialHousingImpact < 0){ 
                    lineColor = 'red';
                    fillColor = 'rgb(240,50,50)';
                }else if (pt.socialHousingImpact == 0){
                    lineColor = 'rgb(200,50,160)';
                    fillColor = 'rgb(200,50,160)';
                } 
                if (pt.status_rc == 'COMPLETED'){
                    opac = 0.7;
                }   
                var shig = Math.sqrt(pt.socialHousingImpact * pt.socialHousingImpact);
        ...
        }else if(crit.legend = "proportion_sh"){
